...
| `app/core/`   | Configuration and infrastructure | Centralizes settings (`config.py`) , database (`db.py`) and  Shared dependencies (database sessions, auth context, etc.).                                            |
...
- Don't put multiple tables model in one file,
...
- Don't use anything that will be Deprecated(Use recommended by python14 and Pydantic2 )
